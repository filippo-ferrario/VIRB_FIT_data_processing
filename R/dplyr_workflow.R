...
data_long <- data %>%
  gather(key,val,-ï..Type,-Local.Number,-Message,-rowID) %>% 
  mutate(
...
calibration <- data_long %>% 
  filter(Message=='three_d_sensor_calibration'&ï..Type=='Data') %>% 
  ...
  select(-rowID,-ï..Type,-Message,-Local.Number)




master <- data_long %>% 
  filter(Message %in% c('gyroscope_data','accelerometer_data','magnetometer_data')&ï..Type=='Data') %>% 
...
unnested$real_value <- apply(unnested,1,function(r){
  # browser()
  ...
  ls <- c(as.integer(r['x']),as.integer(r['y']),as.integer(r['z']))
